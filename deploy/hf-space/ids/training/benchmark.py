...
def benchmark_model(model: torch.nn.Module, preprocessor, X_unscaled_pool: np.ndarray,
                    batch_size: int, n_warmup: int, n_runs: int,
                    device: torch.device, seed: int = 42):
    """End-to-end latency benchmark: preprocessor.transform + tensor build + model forward.

    Args:
        model: Trained PyTorch model
        preprocessor: Fitted Preprocessor (contains scaler + medians)
