...
\par The model emits logits; these are divided by the temperature scalar fitted during calibration before a softmax along the class dimension produces per-class probabilities; the argmax selects the predicted class index; the encoder's inverse transform maps the index back to the human-readable label. The maximum temperature-scaled softmax probability is reported as confidence, so the score shown beside each verdict reflects the model's empirical accuracy rather than the raw, typically over-confident, softmax peak.
...
\par The React frontend connects to the REST endpoint and presents a polished cyberpunk-themed dashboard under the branding \emph{NEURAL IDS}. Its main screen (Figure~\ref{fig:app_dashboard}) lets the operator pick a trained model variant, upload traffic, and review an analysis history table that records the timestamp, model label, filename, dominant predicted class, and confidence of each classification run during the session. The deployed backend serves the MLP variants; the Random Forest baseline of Chapter~\ref{chap:results} is trained and evaluated offline only and is not exposed through the serving path.
...
\par Authentication exists because the demonstration is public and each user's analysis history must stay private to them; Supabase is used because it provides managed authentication together with a Postgres database and row-level security, which removes the need to build and host a separate authentication service. Authentication uses Supabase's email/password provider. On sign-in the client receives a JSON Web Token that is attached to subsequent storage requests, and the session is restored on reload from the persisted token. After each classification, the results screen writes a single row into the \texttt{analyses} table, tagged with the authenticated user's identifier, and the dashboard reads the same table back to render the history list. Each row holds only derived result metadata, never the uploaded capture or CSV: a surrogate UUID key, the owning \texttt{user\_id}, a server-side timestamp, the classifier variant and classification granularity, the input modality (CSV or packet capture), the uploaded file name, the dominant predicted label with its calibrated confidence, and the full per-class probability vector as JSON. This keeps the persistence tier consistent with FR-D5. Per-user isolation is not enforced in client code: a row-level-security policy restricts every \texttt{select} and \texttt{insert} to rows whose \texttt{user\_id} matches the caller's authenticated identifier, so one user can never read another's history even though all rows share a single table.
