...
\par The same random split and evaluation protocol were applied to a Random Forest baseline. Figure~\ref{fig:weighted_f1_splits} compares weighted F1 across the three data partitions for both models; accuracy and macro F1 follow the same train-to-test pattern and are reported in Table~\ref{tab:model_comparison_table}.
...
\begin{figure}[htbp]
    ...
    \begin{minipage}[t]{0.50\textwidth}
        ...
        \caption{Weighted F1 across train, validation, and test partitions for both models on the random split (8-class); full metrics in Table~\ref{tab:model_comparison_table}.}
...
\par NFR-2 (at least 0.85 weighted F1 on the \emph{binary} test set) is satisfied by both models, which reach $\approx$0.90 (MLP) to $\approx$0.92 (Random Forest). At the 8-class granularity no model reaches 0.85 (the best attain $\approx$0.77), which the design treats as the ceiling imposed by a transport-layer feature set rather than a target failure, in keeping with the secondary status assigned to the 8-class task in NFR-2. NFR-5 (faithful reporting) is respected by reporting per-class metrics rather than only aggregate numbers, so that the minority-class weakness is visible without requiring the reader to look past a high aggregate figure, and by flagging the random split's within-session redundancy (Section~\ref{subsec:split_strategies}).
