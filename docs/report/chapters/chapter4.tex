...
\par The F1 functional test specified in Section~\ref{subsec:f1_functional_test} was executed against the deployed serving subsystem described above, using the temporal-split binary-granularity variant. Two captures with known ground-truth labels were submitted: a short capture of normal browsing traffic produced on the demo machine, and a capture of a SYN-flood attack directed at a local target produced with a standard packet-crafting tool. Figure~\ref{fig:f1_terminal} shows the application after startup, with the discovered artefact triples bound to the variant dropdown and the user-interface URL printed on stdout. Figures~\ref{fig:f1_benign} and~\ref{fig:f1_attack} show the user interface immediately after each capture has been classified.

\par For the benign capture, the predicted-label distribution reported in the summary line is dominated by the Benign class, exceeding the 95\% threshold defined in Section~\ref{subsec:f1_functional_test} and discharging the no-false-alarm half of the acceptance criterion. For the SYN-flood capture, the distribution is dominated by the Attack class at the binary granularity (and, when the same capture is re-classified under the attack-family variant, by the DDoS family), discharging the discrimination half of the criterion. Both observed margins sit well above the threshold and well below the corresponding test-set figures, consistent with the deliberately conservative threshold setting that tolerates the distribution shift between training data and the demo machine while still failing loudly under a misconfigured artefact triple.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/f1_terminal.png}
    \caption{Application startup. The discovery probe locates the trained artefact triples on disk and binds the user-interface URL printed on stdout.}
...
\begin{figure}[htbp]
    ...
    \caption{F1 acceptance --- benign capture classified under the temporal-split binary-granularity variant. The summary line and verdict table report Benign as the dominant predicted label, discharging the no-false-alarm half of the F1 acceptance criterion (Section~\ref{subsec:f1_functional_test}).}
    \label{fig:f1_benign}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{F1 acceptance --- synthetic SYN-flood capture classified under the same variant. The summary line and verdict table report Attack as the dominant predicted label, discharging the discrimination half of the F1 acceptance criterion.}
    \label{fig:f1_attack}
\end{figure}
