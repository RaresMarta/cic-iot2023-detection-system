...
\par Authentication uses Supabase's email/password provider. On sign-in the client receives a JSON Web Token that is attached to subsequent storage requests, and the session is restored on reload from the persisted token. After each classification, the results screen writes a single row into the \texttt{analyses} table (Table~\ref{tab:analyses_schema}), tagged with the authenticated user's identifier; the dashboard reads the same table back to render the history list. Crucially, the row carries only \emph{derived result metadata} (never the uploaded capture or CSV), so the persistence tier remains consistent with FR-D5. Per-user isolation is not enforced in client code: a row-level-security policy on the table restricts every \texttt{select} and \texttt{insert} to rows whose \texttt{user\_id} matches the caller's authenticated identifier, so one user can never read another's history even though all rows share a single table.

\begin{table}[htbp]
\begin{center}
\begin{tabular}{|p{145pt}|p{220pt}|}
\hline
\textbf{Column} & \textbf{Meaning} \\
\hline
\hline
\texttt{id} & Surrogate primary key (UUID). \\
\hline
\texttt{user\_id} & Owning user; foreign key to the authentication identity and the predicate of the row-level-security policy. \\
\hline
\texttt{created\_at} & Server-side timestamp of the classification. \\
\hline
\texttt{model\_used} & Classifier variant that produced the verdict (MLP, Random Forest, or XGBoost). \\
\hline
\texttt{input\_type} & Input modality: pre-extracted CSV or raw packet capture. \\
\hline
\texttt{file\_name} & Uploaded file name, or null. \\
\hline
\texttt{result\_label} & Dominant predicted class for the submission. \\
\hline
\texttt{confidence} & Confidence of the dominant class (maximum calibrated softmax probability). \\
\hline
\texttt{classification\_mode} & Granularity of the run (binary or attack-family). \\
\hline
\texttt{probabilities} & Full per-class probability vector, stored as JSON. \\
\hline
\end{tabular}
\end{center}
\caption{Schema of the \texttt{analyses} table. Every column is derived result metadata; no raw traffic payload is stored, keeping the persistence tier consistent with FR-D5.}
\label{tab:analyses_schema}
\end{table}
...
\begin{itemize}
    \item An assertion that the fine-grained label mapping covers every attack folder exactly once, which guards against a missing or duplicated entry.
    \item An assertion during ingestion that no folder is unmapped, which guards against a new attack folder being silently dropped.
